...
class Long_Short_term_Memory(nn.Module):
    ...
    def forward(self, x):
        ...
        cell_state = torch.zeros(self.n_layers, x.size(0), self.hidden_size).to(device)
...
# create NN
model = Long_Short_term_Memory(input_shape, hidden_size, n_layers, sequence_length, num_classes).to(device)
